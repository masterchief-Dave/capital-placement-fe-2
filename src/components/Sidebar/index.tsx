...
export const Sidebar = () => {
  ...
  return (
    <div className="bg-white w-[7.2rem] p-[2.4rem] h-screen flex flex-col items-center justify-between gap-[3.2rem] shadow-sidebar-box-shadow">
      <div className='flex flex-col gap-[3.2rem] items-center'>
        ...
        <div className='flex flex-col gap-[3.2rem]'>
          {sidebarIcons.map((icon, index) => {
            return (
              <div key={index} className={`rounded-[.8rem] ${icon === '/src/assets/House_01.svg' ? 'bg-[#E9EFFF]' : ''}`}>
                <Icon img={icon} />
              </div>
            )
          })}
